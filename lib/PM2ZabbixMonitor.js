 ...
 * @param {number} [options.processListInterval=60000] - The interval (ms) between sending entire process lists to Zabbix. This does not impact process status change updates, which are sent immediately. Meaningless if "monitor" is not enabled.
 * @param {number} [options.processManagerInterval=60000] - The interval (ms) between sending the PM2 supervisor process status to Zabbix. Does nothing when monitor is off.
 */
function PM2ZabbixMonitor(tracker, dataProvider, options) {
	...
	this._options.processListInterval = this._options.processListInterval || 60000;
	this._options.processManagerInterval = this._options.processManagerInterval || 60000;
